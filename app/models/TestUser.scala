...
object UserTypes extends Enumeration {
  ...
  def from(userType: String) = UserTypes.values.find(
    e => e.toString == userType.toUpperCase
  )
}
...
sealed class TestUserMapper(fieldDefinitions: Seq[FieldDefinition]) {
  ...
  protected def asFields(rawFields: Map[String, JsValue]): immutable.Iterable[Field] = rawFields.map {
    f =>
      Field(f._1,
            fieldDefinitions
              .find(
              ...
              .getOrElse(FieldDefinition(f._1, f._1, Seq()))
              .name,
            f._2.toString().stripPrefix("\"").stripSuffix("\"")
      )
  }

  protected def withoutCredentials(fields: Map[String, JsValue]): Map[String, JsValue] =
    fields.filter(
      f => f._1 != "userId" && f._1 != "password"
    )
}
