...
class SuppressedLogFilter(val messagesContaining: String) extends Filter[ILoggingEvent] {
  ...
  def hasError(msg: String) =
    suppressedEntries.exists(
      entry => entry.getLevel == Level.ERROR && entry.getMessage.contains(msg)
    )

  def hasWarn(msg: String) =
    suppressedEntries.exists(
      entry => entry.getLevel == Level.WARN && entry.getMessage.contains(msg)
    )

  def hasInfo(msg: String) =
    suppressedEntries.exists(
      entry => entry.getLevel == Level.INFO && entry.getMessage.contains(msg)
    )
}
...
trait LogSuppressing {
  ...
  def withSuppressedLoggingFrom(logger: Logger, messagesContaining: String)(body: (=> SuppressedLogFilter) => Unit): Unit = {
    ...
    val appendersWithFilters = appenders.map(
      appender => appender -> appender.getCopyOfAttachedFiltersList
    )
    ...
    try body(filter)
    finally appendersWithFilters.foreach {
